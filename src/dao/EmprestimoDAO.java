...
public class EmprestimoDAO {
    ...
    public void save(Emprestimo emprestimo) throws Exception {
        String SQL = "INSERT INTO EMPRESTIMO(EMPRESTIMO_ID, LIVRO_ID, ESTUDANTE_ID, DATA_RETIRADA, DATA_DEVOLUCAO, DATA_ENTREGA, STATUS) VALUES(?, ?, ?, ?, ?, ?, ?)";
        try {
            ...
            p.setDate(4, dt);
            p.setDate(5, emprestimo.getDataDevolucao());
            p.setDate(6, emprestimo.getDataEntrega());
    ...
    public void update(Emprestimo emprestimo) throws Exception {
        String SQL = "UPDATE EMPRESTIMO SET LIVRO_ID=?, ESTUDANTE_ID=?, DATA_DEVOLUCAO=?, DATA_ENTREGA=?, STATUS=? WHERE EMPRESTIMO_ID=?";
        PreparedStatement p = connection.prepareStatement(SQL);
        p.setInt(1, emprestimo.getLivroId());
        p.setInt(2, emprestimo.getEstudanteId());
        p.setDate(3, emprestimo.getDataDevolucao());
        p.setDate(4, emprestimo.getDataEntrega());
    ...
    public List<Emprestimo> findAll() throws Exception {
        ...
        Emprestimo objeto;
        String SQL = "SELECT * FROM EMPRESTIMO ORDER BY EMPRESTIMO_ID";
        try {
            PreparedStatement p = connection.prepareStatement(SQL);
            // 
